...
namespace td_api = td::td_api;
 /// TODO: integrate with future FUSE-based VFS

 /// TODO: remove everything unneeded and leave only the file part
...
public:
        ...
        while (true) {
            if (need_restart_) {
                ...
                if (action == "u") {
                    ...
                    send_file(get_chat_id(), "/home/marat/xd.img");
                }
                    ...
                    deleter->chat_id_ = get_chat_id();
                    send_query(std::move(deleter), {});
                }
                else if (action == "edgar") {
                    /// TODO: Make a easy file upload using chat above
                    get_chat_history(get_chat_id());
                    //send_file(get_chat_id(), "/home/marat/xd.img");
                    // example chat: 430147995
                }
            }
        }
    }

    void get_chat_history(long chat_id ) {
        auto history = td_api::make_object<td_api::getChatHistory>();
        history->chat_id_ = chat_id;
        history->limit_ = 10;
        history->only_local_ = false;

        send_query(std::move(history), [this](Object object) {
                        if (object->get_id() == td_api::error::ID) {
                            std::cerr << "Error, while trying to get chat history.\n";
                            return;
                        }
                        ...
                        for (int i = 0; i < mes->total_count_; i++) {
                            std::cout << mes->messages_[i]->id_ << std::endl;
                        }
                    });
        while (true) {
            auto response = client_manager_->receive(4);
            if (response.object) {
                process_response(std::move(response));
            } else {
                break;
    ...
    void send_file(long chat_id, const std::string& path) {
    ...
    long get_chat_id() {
        ...
        find_chat->username_ = "tg_cloudfilesbot";
        long id = 0;
        send_query(std::move(find_chat), [this, &id](Object object) {
...
private:
    ...
    void on_authorization_state_update() {
        ...
        /// TODO : understand what's happing here :)
        ...
        td_api::downcast_call(*authorization_state_,
                              overloaded(
                                      ...
                                      [this](td_api::authorizationStateWaitTdlibParameters &) {
                                          ...
                                          request->api_id_ = 26859420; // hide
                                          request->api_hash_ = "8aebd2bf31d7edfd742f09928fa12d7e"; // hide
