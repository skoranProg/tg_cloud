...
void TableLoader::LoadInTable(std::unordered_map<fuse_ino_t, uint64_t>& table) {
    ...
    auto gen_insert = [](fuse_ino_t ino, uint64_t mes_id) {
        return  "INSERT INTO DIRECTORIES (INO, MES_ID) VALUES (" + std::to_string(static_cast<int64_t>(ino)) + ", " + std::to_string(static_cast<int64_t>(mes_id)) + ");";
    };
    std::string sql;
...
td::tl_object_ptr<td_api::file> DownloadFileFromMes(TdClass *td_class, td::tl_object_ptr<td_api::message> mes) {
    auto fl = td::move_tl_object_as<td_api::messageDocument>(mes->content_);
    return td_class->DownloadFile(fl->document_->document_->id_);
}

int TableTdUpdater::upload_table(std::unordered_map<fuse_ino_t, uint64_t> &messages) const {
    /// TODO: handle errors
    ...
    td_client_->PinMessage(td_client_->GetMainChatId(), td_client_->GetLastMessage(td_client_->GetMainChatId())->id_);
...
std::unordered_map<fuse_ino_t, uint64_t> TableTdUpdater::get_table() const {
    ...
    auto last_mes = td_client_->GetLastPinnedMessage(td_client_->GetMainChatId());
