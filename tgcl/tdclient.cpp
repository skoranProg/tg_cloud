...
td::tl_object_ptr<td_api::message> TdClass::GetLastMessage(
    const td_api::int53 chat_id) {
...
td::tl_object_ptr<td_api::file> TdClass::DownloadFileFromMes(
...
td_api::int53 TdClass::SendFile(td_api::int53 chat_id, const std::string &path,
    ...
    SendQuery(std::move(send_message), [this, &wait, &file_id, &result_mes_id,
        ...
        this->completed_uploads_[file_id] =
...
void TdClass::SendQuery(td_api::object_ptr<td_api::Function> f,
        ...
        *update,
        overloaded(
            ...
            [this](td_api::updateFile &update_file) {
                completed_downloads_[update_file.file_->id_] =
                    update_file.file_->local_->is_downloading_completed_;
                completed_uploads_[update_file.file_->id_] =
                    update_file.file_->remote_->is_uploading_completed_;
            },
            [this](td_api::updateMessageSendSucceeded &update_message_state) {
                sent_message_[update_message_state.old_message_id_] =
                    update_message_state.message_->id_;
                sent_message_[update_message_state.message_->id_] =
                    update_message_state.message_->id_;
            },
            [](auto &update) {}));
}
        ...
        *authorization_state_,
        overloaded(
            ...
            [this](td_api::authorizationStateWaitRegistration &) {
                ...
                std::cin >> last_name;
                SendQuery(td_api::make_object<td_api::registerUser>(
...
td::tl_object_ptr<td_api::message> TdClass::GetMessage(
    td_api::int53 chat_id, td_api::int53 message_id) {
    ...
    for (auto &s : stop_options) {
        if (strcmp(argv[0], s.c_str()) == 0) {
            if (strcmp(argv[0], "--help") == 0 || strcmp(argv[0], "-h") == 0 ||
                strcmp(argv[0], "-hv") == 0) {
                std::cout << "    --key [path]           path to encryption "
                             "keys\n    --cache_dir [path]     directory, "
                             "where cache files can be stored\n    -api_hash "
