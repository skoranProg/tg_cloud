...
int AES_file_encryptor::encrypt(const std::string &path,
                                const std::string &output) {
    try {
        GCM<AES>::Encryption e;
        e.SetKeyWithIV(keys_->key, keys_->key.size(), keys_->iv,
                       keys_->iv.size());
...
int AES_file_encryptor::decrypt(const std::string &path,
                                const std::string &output) {
    try {
        GCM<AES>::Decryption d;
        d.SetKeyWithIV(keys_->key, keys_->key.size(), keys_->iv,
                       keys_->iv.size());

        AuthenticatedDecryptionFilter df(
            d, new FileSink(output.c_str(), true),
...
int Encryption_Keys::LoadFromFile(const std::string &path) {
    ...
    file.read(reinterpret_cast<char *>(iv.begin()),
              size - AES::DEFAULT_KEYLENGTH);
    return 0;
}
